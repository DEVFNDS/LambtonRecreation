...
@WebServlet("/insertcoach")
public class InsCoachServlet extends HttpServlet {

    private static final long serialVersionUID = 1L;

    public InsCoachServlet() {
        super();
    }
        ...
        try {
            if (request.getSession().getAttribute("username") != null) {
                ...
                if (username != null) {
                    ...
                    if (coach.getSportsSpecializedIn() != null || coach.getCoachingExperience() != null
                            || coach.getCertifications() != null) {
                        request.setAttribute("coach", coach);
                        RequestDispatcher dispatcher = request.getRequestDispatcher("/views/coach/coachCard.jsp");
                        dispatcher.forward(request, response);
                    } else {
                        RequestDispatcher dispatcher = request.getRequestDispatcher("/views/coach/coachForm.jsp");
                        dispatcher.forward(request, response);
                    }
                }
            }
        ...
        try {
            if (request.getSession().getAttribute("username") != null) {
                ...
                if (username != null) {
                    ...
                    int status = CoachDao.saveCoach(coach, username);
                    if (status > 0) {
                        request.setAttribute("coach", coach);
